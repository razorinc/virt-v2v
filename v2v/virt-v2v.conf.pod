...
a set of profiles which pre-define a set of output options.
...
A profile is used to group a set of configuration options defining a specific
destination for a conversion. A user can specify a profile in place of the
equivalent command line options.

A profile is defined by an element in virt-v2v.conf which is a child of the
top-level virt-v2v element. For example:
...
A profile is identified by its I<name> attribute. Allowed elements are:
